...
class SearchFlights extends React.Component {
  ...
  filterDataByDate(dep, arr, unfilteredData) {
    ...
    const ans = unfilteredData.filter((e) => {
      return (
        new Date(new Date(new Date(e.departure.time).setHours(new Date(e.departure.time).getHours() -2))) >= new Date(new Date(dep).setHours(0,0,0)) &&
        new Date(new Date(new Date(e.arrival.time).setHours(new Date(e.arrival.time).getHours() -2))) <= new Date(new Date(arr).setHours(23,59,59))
      );
    });
