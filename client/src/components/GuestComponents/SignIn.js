        ...
        if (res.data.auth) {
          ...
          if (JSON.parse(localStorage.getItem("user")).isAdmin)
            ...
            if (localStorage.getItem("path")) {
              this.props.history.push(
                localStorage.getItem("path").substring(21)
              );
            } else this.props.history.push("/");
          }
        } else {
          this.setState({ showMessage: true, error: res.data.message });
        }
      })
      .catch((err) => {
        console.log(err);
          ...
          <div class="overlay-container">
            <div class="overlay">
              <div class="overlay-panel overlay-right">
                <img className="signInImage-bg" src="SignIn.jpg" />
                <div style={{ zIndex: "5" }}>
                  <h1>Hello, Friend!</h1>
                  <p>Enter your personal details and start journey with us</p>
                  <button onClick={this.onClick} class="ghost" id="signUp">
                    Sign Up
                  </button>
                </div>
              </div>
